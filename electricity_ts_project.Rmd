...
```{r}
# Get overall trend by moving average window of 12 XXXXXXCHANGE
...
adf.test(z_t[7:127]) # since we used the two-sided MA, the first 6 and last 6 points are NA
```
...
plot(z_t, xlim = c(2010, 2023),
     main = "Time series white noise predictions from 2022 to 2023")
...
Time series predictions for electricity sales in 2022 (Yt)
```{r}
...
References:
https://www.geo.fu-berlin.de/en/v/soga/Geodata-analysis/time-series-analysis/Seasonal-decompositon/STL-decomposition/index.html#:~:text=In%20R%20the%20stl(),loess%20window%20for%20seasonal%20extraction.

...
```{r}
acf(energy_ts, lag = length(x) - 1) # not sure why the x axis is wrong
...
The mean estimate can be obtained from rec.yw$x.mean as μˆ = 62.26, while the autoregres- sive parameter estimates and their standard errors are accessed with the commands rec.yw$ar and sqrt(rec.yw$asy.var.coef) as φˆ1 = 1.3316(.0422) and φˆ2 = −.4445(.0422). Finally, the variance estimate is obtained from rec.yw$var.pred as σˆ2 = 94.7991

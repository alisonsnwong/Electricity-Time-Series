...
ts.plot(energy_ts, main = "Electricity (Megawatts) Sold with Yearly Average")
```
...
trend = ma(energy_ts, order = 12, centre = T)
plot(as.ts(energy_ts))
lines(trend, col = "red")
...
m_t = t(matrix(data = detrend, nrow = 12))

# Monthly seasonality: use matrix of 12 rows
seasonal = colMeans(m_t, na.rm = T)
...
# Extract white noise
random = energy_ts - trend - seasonal
plot(as.ts(random))

# Plot time series, ACF, histogram of white noise
checkresiduals(random)
```
